...
class RequestHandler(server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif self.path.startswith('/settings.json'):
            self.serve_settings()
    ...
    def do_POST(self):
        if self.path.startswith('/pad'):
            ...
            self.pad.axes = message["axes"]
            self.pad.buttons = message["buttons"]
        elif self.path.startswith('/ConnectedPad'):
            length = int(self.headers['Content-Length'])
            #print(length)
            self.send_response(200)
            self.end_headers()
            message = json.loads(self.rfile.read(length))
            self.pad.mode = message["state"]
        elif self.path.startswith('/settings'):
            length = int(self.headers['Content-Length'])
            message = json.loads(self.rfile.read(length))
            self.rov.settings = message
            print(self.rov.settings)
            self.send_response(200)
            self.end_headers()
        else:
            warning(message='Bad response. {}. Could not find {}'
    ...
    def serve_settings(self):
        content = json.dumps(self.rov.settings).encode('utf-8')
        self.serve_content(content, 'application/json')
...
class WebpageServer(socketserver.ThreadingMixIn, server.HTTPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, stream_output,
                 rov_proxy, keys_proxy, pad_proxy, index_file=None, debug=False,
                 custom_response=None):
        ...
        RequestHandlerClass.pad = pad_proxy
    ...
    with Picamera2() as camera, \
        ...
        try:
            with WebpageServer(server_address=('', server_port),
                ...
                print('Visit the edurovii webpage at {}'.format(server_ip(server_port)))
                s.serve_forever()
        finally:
